...
void Missile::move(float time) {
    ...
    } else if (m_object.getPosition().x >= WINDOW_WIDTH - 2*LAST_POSITION) {
        m_object.move(DIRECTION * time * FASTER_SPEED);
    }
}
...
void Missile::updateCollisionTime(float time) {
    m_countTime += time;
    sf::Vector2f position = sf::Vector2f(WINDOW_WIDTH + LAST_POSITION, PlayerStateManager::instance().getPosition().y  + PlayerStateManager::instance().getOrigin().y + m_missileCount*100);
    if (m_countTime <= 3) {
    ...
    if (m_object.getPosition().x == WINDOW_WIDTH - 2*LAST_POSITION) {
        m_soundMissileAlarm.play();
    } else if (m_object.getPosition().x == WINDOW_WIDTH + LAST_POSITION) {
