...
/**
 * The `PlayGame::run()` function represents the main game loop.
 * It handles user input, updates game state, and renders the game.
 */
void PlayGame::run() {
    ...
    while (m_window->isOpen() && !restartGame) {
        ...
        if (PlayerStateManager::instance().getState() != DeadPlayer && PlayerStateManager::instance().getState() != GameOver) {
            ...
            // Move objects after player death
...
void PlayGame::createObjectMap() {
    ...
    // Set the initial position for the objects
...
void PlayGame::checkIfNeedToClear() {
    ...
    if (m_lastObjectX <= LAST_POSITION) {
        ...
        // Generate a new random map on the board
        m_board.randMap();
    ...
    for (auto& fallingCoins : m_fallingCoins) {
        ...
        if (fallingCoins->getObject().getPosition().x <= LAST_POSITION) {
            // Set the falling coin for deletion
 ...
 * The flame and board are always animated.
 * Paired objects are animated except for the last object if the number of paired objects is even.
 */
void PlayGame::animateObj() {
    ...
    // Animate the single objects
    for (auto& singleObj : m_singleObj) {
        singleObj->animate();
    }
    // Animate the falling coins
...
void PlayGame::moveObj() {
    // Set the lastObjectX to the LAST_POSITION initially
    m_lastObjectX = LAST_POSITION;

    // Move the physics world
    m_world->Step(TIME_STEP, VELOCITY_ITERATIONS, POSITION_ITERATIONS);

    // Move the board backgrounds
    m_board.moveBackgrounds();

    // Get the movement from the board
...
void PlayGame::deathMoveObj() {
    // Disable the flame animation
    m_flame->setInUse(false);
    // Step the physics world forward by 1.5 times the time step
    m_world->Step(1.5 * TIME_STEP, VELOCITY_ITERATIONS, POSITION_ITERATIONS);
    // Move the player based on the background movement
