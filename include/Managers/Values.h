#pragma once
#include <SFML/Graphics.hpp>
#include <iostream>
#include <unordered_set>

const std::string PATH = "../../../";
//const std::string PATH = "";

//------------------------------ Enums definitions ------------------------------
enum Buttons {
    PlayButton,
    ShopButton,
    SettingButton,
    HelpButton,
};

enum EventsTypes {
    CollectedMoney,
    DeathInTheAir,
    DeadOnTheGround,
    CollectedPiggy,
    startSuperPower,
    ReturnRegular,
};

enum Box2Objects {
    B2Floor,
    B2Ceiling,
    B2Player,
    B2StaticCoin,
    B2DynamicCoin,
};

enum PlayerState {
    Regular,
    DeadPlayer,
    GameOver,
    SuperPowerTank,
    SuperPowerRunner,
};

enum SuperPowersTypes {
    Box,
    WalkingTank,
    FlyingTank,
    Runner,
};

enum NonCollisionTypes {
    Flames,
    Lights,
    Scientists,
};

//------------------------------ Consts definitions -----------------------------
//Chars:
const char COIN       = '*';
const char LASER      = '<';
const char MISSILE    = '!';
const char PIGGY      = '$';
const char SUPERPOWER = '@';

//Floats:
const float WINDOW_HEIGHT   = sf::VideoMode::getDesktopMode().height * 0.9f;
const float WINDOW_WIDTH    = sf::VideoMode::getDesktopMode().width * 0.9f;
const float REFERENCE_HEIGHT= 1800;
const float HEIGHT_RATIO    = sf::VideoMode::getDesktopMode().height / REFERENCE_HEIGHT;

const float WIDTH_CENTER    = WINDOW_WIDTH  / 2;
const float HEIGHT_CENTER   = WINDOW_HEIGHT / 2;
const float PLAYER_POS_X    = WINDOW_WIDTH  / 6;
const float PLAYER_POS_Y    = WINDOW_HEIGHT / 4;
const float FLOOR_POS_Y     = WINDOW_HEIGHT / 9;
const float CEILING_POS_Y   = WINDOW_HEIGHT / 10;
const float GAME_SETTING_X  = 11 * WINDOW_WIDTH / 12;
const float SHOP_POS_X      = 8  * WINDOW_WIDTH / 10;
const float SHOP_POS_Y      = WINDOW_HEIGHT / 10;
const float GAME_SETTING_Y  = WINDOW_HEIGHT / 12;

const float PLAYER_SCALE    = WINDOW_HEIGHT / 1200;
const float OBJECT_SCALE    = WINDOW_HEIGHT / 1000;
const float TITLE_POSITION  = WINDOW_HEIGHT / 4.7;
const float MENU_WIDTH_GAP  = WINDOW_WIDTH  / 8;
const float MENU_HEIGHT_GAP = WINDOW_HEIGHT / 4;
const float SET_BUTTONS     = WINDOW_HEIGHT / 1050;

const float SETTING_CIRCLE  = WINDOW_WIDTH  / 100;
const float SETTING_WIDTH   = WINDOW_WIDTH  / 2;
const float SETTING_HEIGHT  = WINDOW_HEIGHT / 20;
const float SCALE_SIZE      = WINDOW_WIDTH  / 35;

const float GRAVITATION_Y   = 9.8 * HEIGHT_RATIO; //WINDOW_HEIGHT / 500;
const float GRAVITATION_X   = 0.f;
const float DEATH_GRAVITY_Y = GRAVITATION_Y / 3;
const float DEATH_GRAVITY_X = GRAVITATION_Y / 10;
const float BERRYS_MASS     = WINDOW_HEIGHT / 100;
const float COINS_MASS      = WINDOW_HEIGHT / 1000;
const float BERRYS_FRICTION = WINDOW_WIDTH  / 150;
const float TIME_STEP       = std::pow(HEIGHT_RATIO, 4) / (GRAVITATION_Y*5);
const float ANTI_GRAVITY    = -5.f * GRAVITATION_Y;

const float LAST_POSITION = - 100.f;
const float FASTER_SPEED    = 1.5f;
const float CHANGE_INTERVAL = 3.f;
const float ACCELERATION    = 45.f;
const float MINUTE          = 60.f;
const float VELOCITY_DELTA  = 3 / WINDOW_HEIGHT;
const int NUM_OF_BUTTONS    = 4;
const int BACKGROUND        = 3;
const int GAME_SETTINGS     = 3;
const int NUM_OF_OBJECTS    = 15;
const int COLLECTED_MONEY   = 1;
const int COLLECTED_PIGGY   = 10;
const int COLLECTED_SUPER   = 30;
const int TOP_FIVE          = 5;

//Double:
const double PI             = 3.14159265358;

//Vector2f:
const sf::Vector2f DEFAULT_VEC                 = sf::Vector2f(0.f, 0.f);
const sf::Vector2f DIRECTION                   = sf::Vector2f (-1, 0);

//Vectors:
const std::vector<int> prices                  = {0, 500, 850, 1200};
const std::vector<std::string> scoreBoard      = {"MONEY: ","TIME: ", "BEST: "};
const std::vector<std::string> squareNames     = {"Berry", "King","Scientist" , "Soldier"};

const std::vector<std::string> buttons         = {"PlayGameOff.png", "ShopOff.png", "SettingsOff.png", "HelpOff.png", "PlayGameOn.png", "ShopOn.png", "SettingsOn.png", "HelpOn.png"};
const std::vector<std::string> missile         = {"MissileWarning.png","MissileIncoming.png", "Missile.png"};
const std::vector<std::string> superPower      = {"PowerUpBox.png", "SuperPowerTank.png" ,"SuperPowerFlyTank.png", "SuperPowerRunner.png"};
const std::vector<std::string> help            = {"HelpMenuOne.png", "HelpMenuTwo.png" , "HelpMenuThree.png"};
const std::vector<std::string> settingsButtons = {"Resume.png", "Retry.png", "SettingQuit.png"};
const std::vector<std::string> playersAvatar   = {"BerryAvatar.png", "ScientistAvatar.png" ,"SoldierAvatar.png", "KingAvatar.png"};
const std::vector<std::vector<std::string>> playerStates = { {"BerryWalking.png", "BerryHit.png", "BerryDead.png"},
                                                             {"ScientistWalking.png", "ScientistHit.png", "ScientistDead.png"},
                                                             {"SoldierWalking.png", "SoldierHit.png", "SoldierDead.png"},
                                                             {"KingWalking.png", "KingHit.png", "KingDead.png"} };

const std::vector<std::string> MAP = { "---**----------\n"
                                       "--****---------\n"
                                       "-******--------\n"
                                       "-*******-------\n"
                                       "--*******------\n"
                                       "-*******-------\n"
                                       "-******--------\n"
                                       "--****--------<\n"
                                       "---**----------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "--------------<\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "-------<-----<-\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "------@--------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "-<-------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "-<-------------\n"
                                       "-------**------\n"
                                       "------****-----\n"
                                       "-----******----\n"
                                       "-----*******---\n"
                                       "------*******--\n"
                                       "-----*******---\n"
                                       "-----******----\n"
                                       "------****--<--\n"
                                       "-------**------\n"
                                       "---------------\n"
                                       "------------<--\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "--<-----<------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "-----@---------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "-<-------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "-<-------------\n"
                                       "-------!-------" };

/*
const float WIDTH_CENTER    = WINDOW_WIDTH  / 2.f;
const float HEIGHT_CENTER   = WINDOW_HEIGHT / 2.f;
const float PLAYER_POS_X    = WINDOW_WIDTH  / 6.f;
const float PLAYER_POS_Y    = WINDOW_HEIGHT / 4.f;
const float FLOOR_POS_Y     = WINDOW_HEIGHT / 9.f;
const float CEILING_POS_Y   = WINDOW_HEIGHT / 10.f;
const float GAME_SETTING_X  = 11 * WINDOW_WIDTH / 12.f;
const float SHOP_POS_X      = 8  * WINDOW_WIDTH / 10.f;
const float SHOP_POS_Y      = WINDOW_HEIGHT / 10.f;
const float GAME_SETTING_Y  = WINDOW_HEIGHT / 12.f;
const float VELOCITY_DELTA  = 3 / WINDOW_HEIGHT;

const float PLAYER_SCALE    = WINDOW_HEIGHT / 1200.f;
const float OBJECT_SCALE    = WINDOW_HEIGHT / 1000.f;
const float TITLE_POSITION  = WINDOW_HEIGHT / 5.f;
const float MENU_WIDTH_GAP  = WINDOW_WIDTH  / 8.f;
const float MENU_HEIGHT_GAP = WINDOW_HEIGHT / 4.f;
const float SET_BUTTONS     = WINDOW_HEIGHT / 1050.f;

const float SETTING_CIRCLE  = WINDOW_WIDTH  / 100.f;
const float SETTING_HEIGHT  = WINDOW_HEIGHT / 20.f;
const float SCALE_SIZE      = WINDOW_WIDTH  / 35.f;

const float GRAVITATION_Y   = WINDOW_HEIGHT / 500.f;
const float GRAVITATION_X   = 0.f;
const float DEATH_GRAVITY_Y = WINDOW_HEIGHT / 1500.f;
const float DEATH_GRAVITY_X = WINDOW_WIDTH  / 7500.f;
const float BERRYS_MASS     = WINDOW_HEIGHT / 100.f;
const float COINS_MASS      = WINDOW_HEIGHT / 1000.f;
const float BERRYS_FRICTION = WINDOW_WIDTH  / 150.f;
const float TIME_STEP       = WINDOW_HEIGHT / 40000.0f;

const float ANTI_GRAVITY    = -10.f;
const float CHANGE_INTERVAL = 3.f;
const float ACCELERATION    = 45.f;
const float MINUTE          = 60.f;
*/

/*
const std::vector<std::string> MAP = { "--*---------*--\n"
                                       "--*---------*--\n"
                                       "---*-------*---\n"
                                       "----*-----*<---\n"
                                       "-----*---*-----\n"
                                       "------*-*------\n"
                                       "-------*-------\n"
                                       "------*-*------\n"
                                       "-----*---*-<---\n"
                                       "----*-----*----\n"
                                       "---*-------*---\n"
                                       "--*---------*--\n"
                                       "--*---------*--\n"
                                       "--*--------*---\n"
                                       "---*------*----\n"
                                       "--<-*----*-----\n"
                                       "-----*--*------\n"
                                       "------**-------\n"
                                       "------**-------\n"
                                       "-----*--*------\n"
                                       "----*----*-----\n"
                                       "--<*------*----\n"
                                       "---*-------*---\n"
                                       "--*---------*--\n"
                                       "--*---------*--\n"
                                       "---*--------*--\n"
                                       "----*------*---\n"
                                       "-----*----*----\n"
                                       "------*--*-----\n"
                                       "-------**------\n"
                                       "-------**------\n"
                                       "---!---$-------",

                                       "--*--*---*--*--\n"
                                       "--*--*---*--*--\n"
                                       "--*--*@--*--*--\n"
                                       "--*--*---*--*--\n"
                                       "--*--*---*--*--\n"
                                       "--***********--\n"
                                       "--**********---\n"
                                       "--**-------**--\n"
                                       "---**-----**---\n"
                                       "----**---**----\n"
                                       "-----**-**-----\n"
                                       "------***------\n"
                                       "-------*-------\n"
                                       "-------$-------",

                                       "----***--------\n"
                                       "---**-**-------\n"
                                       "--**---**<-----\n"
                                       "-**-----**-----\n"
                                       "**-------**----\n"
                                       "**--------**---\n"
                                       "-**------<-**--\n"
                                       "--**----@---**-\n"
                                       "---**--------**\n"
                                       "<-**--------**-\n"
                                       "-**--------**--\n"
                                       "**--------**---\n"
                                       "**-------**----\n"
                                       "<**-----**-----\n"
                                       "--**---**------\n"
                                       "---**-**-------\n"
                                       "----***----!---",

                                       "-------**----**\n"
                                       "-------**----**\n"
                                       "-------**----**\n"
                                       "-------**----**\n"
                                       "--------**--**-\n"
                                       "---------****--\n"
                                       "----------**---\n"
                                       "**----**-------\n"
                                       "**----**-------\n"
                                       "**----**-------\n"
                                       "**----**-------\n"
                                       "-**--**--------\n"
                                       "--****---------\n"
                                       "---**----------\n"
                                       "-----**----**--\n"
                                       "-----**----**--\n"
                                       "-----**----**--\n"
                                       "-----**----**--\n"
                                       "------**--**---\n"
                                       "-------****----\n"
                                       "--------**-----\n"
                                       "--**----**-----\n"
                                       "--**----**-----\n"
                                       "--**----**-----\n"
                                       "--**----**-----\n"
                                       "---**--**------\n"
                                       "----****-------\n"
                                       "-----**----!---",

                                       "----------*****\n"
                                       "--@------*****-\n"
                                       "--<-----*****--\n"
                                       "-------*****---\n"
                                       "------*****----\n"
                                       "-----*****-----\n"
                                       "----*****------\n"
                                       "---*****---!---\n"
                                       "--<****--------\n"
                                       "-*****---------\n"
                                       "*****----------\n"
                                       "----------*****\n"
                                       "---------*****-\n"
                                       "--------*****--\n"
                                       "--<----*****---\n"
                                       "------*****----\n"
                                       "-----*****-----\n"
                                       "--<-*****------\n"
                                       "---*****---!---\n"
                                       "--*****--------\n"
                                       "-*****---------\n"
                                       "*****----------",

                                       "*****----------\n"
                                       "-*****---------\n"
                                       "--*****---!----\n"
                                       "---*****-------\n"
                                       "----*****------\n"
                                       "-----*****-----\n"
                                       "------*****----\n"
                                       "-<-----*****---\n"
                                       "--------*****--\n"
                                       "--<------*****-\n"
                                       "----------*****",

                                       "-------*-------\n"
                                       "---<--***------\n"
                                       "-----*****-----\n"
                                       "----**---**----\n"
                                       "---**---<-**---\n"
                                       "--**-------**--\n"
                                       "-**---------**-\n"
                                       "**-----------**\n"
                                       "-**---------**-\n"
                                       "--**-------**--\n"
                                       "---**-----**---\n"
                                       "----**-<-**----\n"
                                       "-----*****-----\n"
                                       "------***------\n"
                                       "-------*-------\n"
                                       "------***---<--\n"
                                       "-----*****-----\n"
                                       "-<--**---**----\n"
                                       "---**-----**---\n"
                                       "--**-------**--\n"
                                       "-**-<-------**-\n"
                                       "**-----------**\n"
                                       "-**---------**-\n"
                                       "--**-------**--\n"
                                       "---**-----**---\n"
                                       "----**---**----\n"
                                       "-----*****-----\n"
                                       "<----<***------\n"
                                       "-------*-------",

                                       "-------------<-\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "--**----<------\n"
                                       "-****----------\n"
                                       "-*****---------\n"
                                       "--*****--------\n"
                                       "-*****---------\n"
                                       "-****----------\n"
                                       "--**-----------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---*****-------\n"
                                       "---*-*-*-------\n"
                                       "--*--*--*------\n"
                                       "-*********-----\n"
                                       "---------------\n"
                                       "--<------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "------<--------\n"
                                       "---------------\n"
                                       "---!-------@---\n"
                                       "---------------\n"
                                       "----****-------\n"
                                       "----****-------\n"
                                       "----****-------\n"
                                       "----****-------\n"
                                       "---------------\n"
                                       "-<------<------\n"
                                       "---------------\n"
                                       "----------$----\n"
                                       "--***----------\n"
                                       "-*---*-------<-\n"
                                       "*-----*--------\n"
                                       "--***----------\n"
                                       "-------------<-\n"
                                       "------**-------\n"
                                       "------**-------\n"
                                       "------**-------",

                                       "-****-----****-\n"
                                       "-****-----****-\n"
                                       "-****-----****-\n"
                                       "-----*****-----\n"
                                       "-----*****-----\n"
                                       "---@-*****-----\n"
                                       "-****-----****-\n"
                                       "-****-----****-\n"
                                       "-****-----****-\n"
                                       "-----*****-----\n"
                                       "-----*****--!--\n"
                                       "-----*****-----\n"
                                       "---------------\n"
                                       "---<-----<-----\n"
                                       "---------------\n"
                                       "-----*-----@---\n"
                                       "----***--------\n"
                                       "----***--------\n"
                                       "----***--------\n"
                                       "-----*---------\n"
                                       "-------*-------\n"
                                       "------***------\n"
                                       "------***------\n"
                                       "------***------\n"
                                       "-------*-------\n"
                                       "---------------\n"
                                       "--<----<-------\n"
                                       "---------!-----" ,

                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------\n"
                                       "---------------" };

const std::vector<std::string> MAP = { "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "---------------*-\n"
                                       "---------------*-\n"
                                       "---------------*-\n"
                                       "--------------*--\n"
                                       "-------------*---\n"
                                       "--@---------*----\n"
                                       "-----------*-----\n"
                                       "----------*------\n"
                                       "---------*-------\n"
                                       "---!----*--------\n"
                                       "-------*---------\n"
                                       "------*----------\n"
                                       "-----*-----------\n"
                                       "----*------------\n"
                                       "---*-------------\n"
                                       "---*-------------\n"
                                       "---*-------------\n"
                                       "----*------------\n"
                                       "-----*-----------\n"
                                       "------*----------\n"
                                       "-------*---------\n"
                                       "--------*--------\n"
                                       "--<------*-------\n"
                                       "----------*------\n"
                                       "-----------*-----\n"
                                       "------------*----\n"
                                       "-------------*---\n"
                                       "--<-----------*--\n"
                                       "---------------*-\n"
                                       "---------------*-\n"
                                       "---------@-----*-\n"
                                       "---------------*-\n"
                                       "--------------*--\n"
                                       "-------------*---\n"
                                       "------------*----\n"
                                       "-----------*-----\n"
                                       "----------*------\n"
                                       "---------*-------\n"
                                       "--------*--------\n"
                                       "-------*---------\n"
                                       "------*--------<-\n"
                                       "-----*-----------\n"
                                       "----*------------\n"
                                       "---*----<--------\n"
                                       "---*-------------\n"
                                       "---*-------------\n"
                                       "----*------------\n"
                                       "-----*-----------\n"
                                       "------*----------\n"
                                       "-------*------!--\n"
                                       "--------*--------\n"
                                       "---------*-------\n"
                                       "----------*------\n"
                                       "-----------*-----\n"
                                       "-<----------*----\n"
                                       "-------------*---\n"
                                       "--------------*--\n"
                                       "--------<------*-\n"
                                       "---------------*-\n"
                                       "---------------*-\n"
                                       "--------------*--\n"
                                       "-------------*---\n"
                                       "------------*----\n"
                                       "-----------*-----\n"
                                       "----------*------\n"
                                       "---------*-------\n"
                                       "--------*--------\n"
                                       "-------*---------\n"
                                       "------*----------\n"
                                       "-----*-----------\n"
                                       "--!--$-----------",

                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "---**------------\n"
                                       "--****-----------\n"
                                       "-******----------\n"
                                       "-*******---------\n"
                                       "--*******--------\n"
                                       "-*******---------\n"
                                       "-******----------\n"
                                       "--****--------<--\n"
                                       "---**------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "--------------<--\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "---------<-----<-\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "------@----------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-<---------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "------!----------\n"
                                       "-<---------------\n"
                                       "---------**------\n"
                                       "--------****-----\n"
                                       "-------******----\n"
                                       "-------*******---\n"
                                       "--------*******--\n"
                                       "-------*******---\n"
                                       "-------******----\n"
                                       "--------****--<--\n"
                                       "---------**------\n"
                                       "-----------------\n"
                                       "--------------<--\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "--<-----<--------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "------@----------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-<---------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-<---------------\n"
                                       "---------!-------",

                                       "-----------****--\n"
                                       "-----------****--\n"
                                       "-----------****--\n"
                                       "------<----****--\n"
                                       "-----------****--\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-<----****-------\n"
                                       "------****-------\n"
                                       "------****-------\n"
                                       "------****-------\n"
                                       "------****-------\n"
                                       "-----------------\n"
                                       "--------------<--\n"
                                       "-----------------\n"
                                       "-****------------\n"
                                       "-****------------\n"
                                       "-****------------\n"
                                       "-****------------\n"
                                       "-****----<-------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------****--\n"
                                       "-----------****--\n"
                                       "-----------****--\n"
                                       "-----------****--\n"
                                       "-----------****--\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "------****----<--\n"
                                       "------****-------\n"
                                       "------****-------\n"
                                       "------****-------\n"
                                       "------****----<--\n"
                                       "-----------------\n"
                                       "-----!-----------\n"
                                       "-----------------\n"
                                       "-****------------\n"
                                       "-****------------\n"
                                       "-****------------\n"
                                       "-****------------\n"
                                       "-****------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------****--\n"
                                       "-----------****--\n"
                                       "-----------****--\n"
                                       "-----------****--\n"
                                       "-----------****--\n"
                                       "-----------------\n"
                                       "-<----<----------\n"
                                       "-----------------\n"
                                       "------****-------\n"
                                       "------****-------\n"
                                       "------****-------\n"
                                       "------****-------\n"
                                       "------****-------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-****---------<--\n"
                                       "-****------------\n"
                                       "-****------------\n"
                                       "-****---------<--\n"
                                       "-****------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------****--\n"
                                       "-----------****--\n"
                                       "-----------****--\n"
                                       "-----------****--\n"
                                       "-----------****--\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "------****-------\n"
                                       "------****-------\n"
                                       "------****-------\n"
                                       "------****-------\n"
                                       "------****-------\n"
                                       "--------<------<-\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-****------------\n"
                                       "-****------------\n"
                                       "-****------------\n"
                                       "-****------------\n"
                                       "-****------------\n"
                                       "-----------!-----",

                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "------*---------*\n"
                                       "------*---------*\n"
                                       "------*---------*\n"
                                       "------*---------*\n"
                                       "------*---------*\n"
                                       "------*---------*\n"
                                       "------*---------*\n"
                                       "-------*-------*-\n"
                                       "--------*-----*--\n"
                                       "---------*---*---\n"
                                       "----------*-*----\n"
                                       "-----------*-----\n"
                                       "-----------------\n"
                                       "------<--------<-\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----!-----------\n"
                                       "*---------*------\n"
                                       "*---------*------\n"
                                       "*---------*------\n"
                                       "*---------*------\n"
                                       "*---------*------\n"
                                       "*---------*------\n"
                                       "*---------*------\n"
                                       "-*-------*-------\n"
                                       "--*-----*--------\n"
                                       "---*---*---------\n"
                                       "----*-*----------\n"
                                       "-----*-----------\n"
                                       "-----------------\n"
                                       "-<--------<------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "------*---------*\n"
                                       "------*---------*\n"
                                       "------*---------*\n"
                                       "------*---------*\n"
                                       "------*---------*\n"
                                       "------*---------*\n"
                                       "------*---------*\n"
                                       "-------*-------*-\n"
                                       "--------*-----*--\n"
                                       "---------*---*---\n"
                                       "----------*-*----\n"
                                       "-----------*-----\n"
                                       "-----------------\n"
                                       "------<--------<-\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "*---------*------\n"
                                       "*---------*------\n"
                                       "*---------*------\n"
                                       "*---------*------\n"
                                       "*---------*------\n"
                                       "*---------*------\n"
                                       "*---------*------\n"
                                       "-*-------*-------\n"
                                       "--*-----*--------\n"
                                       "---*---*---------\n"
                                       "----*-*----------\n"
                                       "-----*-----------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-<--------<------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------!-----",

                                       "-----------------\n"
                                       "--*-------*------\n"
                                       "--**-----**------\n"
                                       "--***---***------\n"
                                       "--***---***------\n"
                                       "--***---***------\n"
                                       "---*** ***-------\n"
                                       "----*****--------\n"
                                       "-----***---------\n"
                                       "------*----------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "--*-------*------\n"
                                       "--**-----**------\n"
                                       "--***---***------\n"
                                       "--***---***------\n"
                                       "--***---***------\n"
                                       "---*** ***-------\n"
                                       "----*****--------\n"
                                       "-----***---------\n"
                                       "------*----------\n"
                                       "---!-------------\n"
                                       "-----------------\n"
                                       "--*-------*------\n"
                                       "--**-----**------\n"
                                       "--***---***------\n"
                                       "--***---***------\n"
                                       "--***---***------\n"
                                       "---*** ***-------\n"
                                       "----*****--------\n"
                                       "-----***---------\n"
                                       "------*----------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "--*-------*------\n"
                                       "--**-----**------\n"
                                       "--***---***------\n"
                                       "--***---***------\n"
                                       "--***---***------\n"
                                       "---*** ***-------\n"
                                       "----*****-----!--\n"
                                       "-----***---------\n"
                                       "------*----------",

                                       "--*-----------*--\n"
                                       "---*---------*-<-\n"
                                       "----*-------*----\n"
                                       "-----*-----*-----\n"
                                       "------*---*------\n"
                                       "-------*-*-------\n"
                                       "--------*--------\n"
                                       "-------*-*-------\n"
                                       "------*---*------\n"
                                       "-----*-----*-----\n"
                                       "----*-------*----\n"
                                       "---*----!----*-<-\n"
                                       "--*-----------*--\n"
                                       "-<-*---------*---\n"
                                       "----*-------*----\n"
                                       "-----*-----*-----\n"
                                       "------*---*------\n"
                                       "-------*-*-------\n"
                                       "--------*--------\n"
                                       "-------*-*-------\n"
                                       "------*---*------\n"
                                       "-----*-----*-----\n"
                                       "----*-------*----\n"
                                       "-<-*---------*---\n"
                                       "--*-----------*--\n"
                                       "---*---------*-<-\n"
                                       "----*-------*----\n"
                                       "-----*-----*-----\n"
                                       "------*---*------\n"
                                       "-------*-*-------\n"
                                       "--------*--------\n"
                                       "-------*-*-------\n"
                                       "------*---*------\n"
                                       "-----*-----*-----\n"
                                       "----*-------*----\n"
                                       "---*---------*-<-\n"
                                       "--*-----!-----*--",

                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----------------\n"
                                       "-----***---------\n"
                                       "----**-**--------\n"
                                       "---**---**-------\n"
                                       "--**-----**------\n"
                                       "-**-------**-----\n"
                                       "**---------**----\n"
                                       "**----------**---\n"
                                       "-**----------**--\n"
                                       "--**----------**-\n"
                                       "---**---@------**\n"
                                       "--**----------**-\n"
                                       "-**----------**--\n"
                                       "**----------**---\n"
                                       "**---------**----\n"
                                       "-**-------**-----\n"
                                       "--**-----**------\n"
                                       "---**---**-------\n"
                                       "----**-**--------\n"
                                       "-----***-----!---",}; */